...
<body>
  ...
  if (isset($_POST['submit'])) {
    ...
    $sqlquery = "INSERT INTO all_products (name,img,brand,price,delprice,discount,ratings) VALUES ('$productname', '$productimg', '$productbrand', '$productprice', '$productdelprice', '$productdiscount', '$productratings')";


    if (mysqli_query($conn, $sqlquery)) {

      echo "<script>alert('New Product Inserted Successfully.')</script>";
      echo "<script>window.open('mainshop.php','_self')</script>";
    } else {
  ...
  <div class="container pt-4 my-5">
    <form action="" method="POST" class="row g-3 needs-validation border border-dark p-4 rounded mt-5" novalidate>
      <h2 class="text-center pb-1">Hello! <?php echo "  " . $_SESSION['username'] ?></h2>
      <p class="text-center text-success"><strong>Fill all the necessary details of product</strong></p>
      <hr>
      <div class="row">
        <div class="col-lg-6">
      ...
      <div class="col-12">
        <button class="btn btn-dark w-100" type="submit" name="submit">Submit</button>
      </div>
  </div>
  </div>
  </form>
